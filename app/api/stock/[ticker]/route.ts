  ...
  if (cachedTickerData && cachedTickerData.lastFetch === today) {
    ...
    if (API_KEYS.length === 0) {
      return NextResponse.json(
        {
          error:
            "설정된 Alpha Vantage API 키가 없습니다. .env 파일을 확인하세요.",
        },
        { status: 500 },
      );
    }
    ...
    let lastError: string =
      "모든 API 키를 사용했지만 데이터를 가져오지 못했습니다.";

    for (const apiKey of API_KEYS) {
      try {
        console.log(
          `[${ticker}] API Key ...${apiKey.slice(-4)}로 데이터 조회 시도...`,
        );
        const response = await fetch(
          `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${ticker}&outputsize=full&apikey=${apiKey}`,
        );
        if (!response.ok)
          throw new Error(`API 서버 오류! Status: ${response.status}`);
    ...
    if (successfulData && successfulData["Time Series (Daily)"]) {
      ...
      return NextResponse.json(
        {
          error: `'${ticker}' 데이터를 불러오는 데 실패했습니다. 최종 원인: ${lastError}`,
        },
        { status: 500 },
      );
    }
