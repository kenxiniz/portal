...
import {
  generateLottoSets,
  getDrawNoForDate,
  checkAndUpdateLatestWinningNumbers,
} from "@/lib/lottoUtils";
...
const getNextDrawInfo = (): {
  ...
  const dayOfYear = Math.floor(
    (nextSaturday.getTime() - startOfYear.getTime()) / (1000 * 60 * 60 * 24),
  );
...
export async function GET() {
  ...
  const existingDataForNextDraw = Object.values(lottoDb).find(
    (w) => w.drawNo === drawNo,
  );
  if (!existingDataForNextDraw) {
    console.log(
      `[로또 번호 생성] ${drawNo}회차(${week}) 번호가 없어 새로 생성합니다.`,
    );
    ...
    console.log(
      `[로또 번호 생성] 이미 ${drawNo}회차(${week}) 번호가 존재하여 건너뜁니다.`,
    );
  }

  const pastWinningNumbers = (await readJsonFile(
    pastWinningNumbersPath,
  )) as Record<string, { numbers: number[]; bonus: number }>;

  const combinedData = Object.values(lottoDb).map((weekData) => {
    const currentDrawNo =
      weekData.drawNo || getDrawNoForDate(new Date(weekData.drawDate));
    const winningInfo =
      weekData.winningNumbers || pastWinningNumbers[currentDrawNo];
  ...
  const finalData = sortedData.map((weekData) => {
    ...
    const previousWeekWinningNumbers =
      pastWinningNumbers[previousDrawNo] || null;
