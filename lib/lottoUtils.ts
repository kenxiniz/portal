...
export const getWinningNumbers = async (drawNo: number) => {
  try {
    ...
    if (data.returnValue === "success") {
      ...
      throw new Error(
        `[${drawNo}회차] 당첨 번호 조회 실패: ${data.returnValue}`,
      );
    }
  } catch (error) {
...
export const checkAndUpdateLatestWinningNumbers = async () => {
  try {
    ...
    console.log(
      `[로또 로그] 동행복권 API로 최신 ${latestDrawNo}회차 당첨 번호 조회를 시도합니다...`,
    );
    ...
    if (winningNumbers) {
      console.log(
        `[로또 로그] ✅ 조회 성공: ${latestDrawNo}회차 당첨 번호는 [${winningNumbers.numbers.join(", ")}] + 보너스 ${winningNumbers.bonus} 입니다.`,
      );

      const pastNumbers = await readJsonFile(pastWinningNumbersPath);
      if (!pastNumbers[latestDrawNo]) {
        console.log(
          `[로또 로그] ⚠️ ${latestDrawNo}회차 번호가 .cache/past-winning-numbers.json에 없어 새로 추가합니다.`,
        );
        pastNumbers[latestDrawNo] = winningNumbers;
        await writeJsonFile(pastWinningNumbersPath, pastNumbers);
      } else {
        console.log(
          `[로또 로그] ℹ️ ${latestDrawNo}회차 번호는 이미 파일에 존재합니다.`,
        );
      }
    } else {
      console.log(
        `[로또 로그] ℹ️ 아직 ${latestDrawNo}회차 당첨 번호가 발표되지 않았습니다.`,
      );
    }
  } catch (error) {
    console.error(
      "[로또 로그] ❌ 최신 당첨 번호 확인/업데이트 중 오류 발생:",
      error,
    );
  }
};
