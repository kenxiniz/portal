...
if (global.isSchedulerRunning) {
  ...
  /* [삭제] 스케줄러 시작 시 실행되던 로그 함수 및 호출부 제거 */
  ...
  const createDailySummaryListTemplates = (latestSignals: {ticker: string, signal: TradingSignal | null}[]) => {
    ...
    return chunks.map((chunk, index) => ({
      ...
      "header link": { "web_url": `${process.env.NEXTAUTH_URL}/stock`, "mobile_web_url": `${process.env.NEXTAUTH_URL}/stock` },
      "contents": chunk, 
  ...
  cron.schedule('5 8 * * 1', async () => {
    console.log(`매주 월요일 오전 8시 5분: 로또 번호 생성을 시작합니다...`);
    try {
  ...
  cron.schedule('0 8 * * 1', async () => {
    console.log('매주 월요일 오전 8시: 로또 당첨 번호 업데이트를 시작합니다...');
    try {
  ...
  cron.schedule('0 8 * * *', async () => {
    ...
    for (const ticker of tickers) {
      try {
        ...
        latestSignals.push({ ticker, signal: null }); 
      }
    }
