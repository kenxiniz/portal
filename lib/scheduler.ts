...
class KakaoNotificationService {
  ...
  private async _refreshAccessToken(): Promise<boolean> {
    ...
    try {
      ...
      if (response.data.refresh_token) {
        console.warn('⚠️ 새로운 Refresh Token이 발급되었습니다. 환경 변수를 업데이트해야 할 수 있습니다.');
      }
      return true;
    } catch (error) {
  ...
  private async _sendMessage(url: string, data: URLSearchParams, attempt = 1): Promise<void> {
    ...
    try {
      ...
      if (axiosError.response?.data?.code === -401 && attempt === 1) {
        ...
        console.error('❌ 카카오 메시지 전송 최종 실패:', axiosError.response?.data || axiosError.message);
      }
    }
  }
  ...
  public async notify(templateObject: object): Promise<void> {
    ...
    const toMeData = new URLSearchParams({ template_object: JSON.stringify(templateObject) });
  ...
  public async notifyInChunks<T>(createTemplate: (chunk: T[]) => object, items: T[], chunkSize: number): Promise<void> {
  ...
  public createLottoSetsTemplate = (drawNo: number) => (sets: LottoSet[]): object => ({
    ...
    header_link: { web_url: `${config.apiBaseUrl}/lotto`, mobile_web_url: `${config.apiBaseUrl}/lotto` },
    contents: sets.map((set, index) => ({
      title: `${index + 1}번째 조합`,
      description: set.numbers.join(', '),
      image_url: `${config.apiBaseUrl}/lotto.png`,
      link: { web_url: `${config.apiBaseUrl}/lotto`, mobile_web_url: `${config.apiBaseUrl}/lotto` },
    })),
    buttons: [{ title: '전체 번호 확인하기', link: { web_url: `${config.apiBaseUrl}/lotto`, mobile_web_url: `${config.apiBaseUrl}/lotto` } }],
  });

  public createStockStatusTemplate = (signals: { name: string; signal: TradingSignal }[]): object => ({
    object_type: 'list',
    header_title: '🇺🇸 미국 주식 현재 상태',
    header_link: { web_url: `${config.apiBaseUrl}/stock`, mobile_web_url: `${config.apiBaseUrl}/stock` },
    contents: signals.map(item => ({
      ...
      link: { web_url: `${config.apiBaseUrl}/stock`, mobile_web_url: `${config.apiBaseUrl}/stock` },
    })),
    buttons: [{ title: '미국 주식 페이지로 이동', link: { web_url: `${config.apiBaseUrl}/stock`, mobile_web_url: `${config.apiBaseUrl}/stock` } }],
  });
}
...
class JobScheduler {
  ...
  private initializeJobs(): void {
    this._scheduleJob('로또 당첨 번호 업데이트', config.cronSchedules.updateLottoWins, this._updateLottoWinningNumbers);
    this._scheduleJob('일일 로또 번호 발송', config.cronSchedules.sendDailyLotto, this._sendDailyLottoNumbers);
    this._scheduleJob('일일 주식 매매 신호 발송', config.cronSchedules.sendDailyStockSignals, this._sendDailyStockSignals);
  }
  ...
  private async _sendDailyLottoNumbers(): Promise<void> {
    const lottoDb: Record<string, LottoWeek> = JSON.parse(await fs.readFile(config.lottoDbPath, 'utf8'));
    ...
    if (currentWeekData?.generatedSets && currentWeekData.generatedSets.length > 0) {
      console.log(`${currentDrawNo}회차 생성된 번호 ${currentWeekData.generatedSets.length}세트를 발송합니다.`);
      const templateFn = this.kakaoService.createLottoSetsTemplate(currentDrawNo);
      await this.kakaoService.notifyInChunks(templateFn, currentWeekData.generatedSets, config.notificationChunkSize);
    } else {
      console.log(`${currentDrawNo}회차에 해당하는 생성된 번호가 없어 발송을 건너뜁니다.`);
    }
  }
  ...
  private async _sendDailyStockSignals(): Promise<void> {
    ...
    for (const stock of usStocks) {
      try {
        const response = await axios.get(`${config.apiBaseUrl}/api/stock/${stock.ticker}`);
        const { signals }: { signals: TradingSignal[] } = response.data;
        if (signals?.length > 0) {
          allLatestSignals.push({ name: stock.ticker, signal: signals.at(-1)! });
        }
      } catch (error) {
        const axiosError = error as AxiosError;
        console.error(`${stock.ticker} 상태 확인 중 오류:`, axiosError.response?.data || axiosError.message);
      }
    }

    if (allLatestSignals.length > 0) {
      console.log(`${allLatestSignals.length}개의 미국 주식 종목 상태를 확인하여 알림을 발송합니다.`);
      const template = this.kakaoService.createStockStatusTemplate(allLatestSignals);
      await this.kakaoService.notify(template);
    } else {
      console.log('조회할 미국 주식 종목이 없거나 데이터를 가져오는 데 실패했습니다.');
    }
  }
}
