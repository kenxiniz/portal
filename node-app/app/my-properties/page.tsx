    ...
    <div className="flex flex-col items-center p-4 md:p-8 bg-slate-100 dark:bg-slate-950 min-h-screen">
      ...
      >
        {owners.map((owner) => {
          ...
          const comprehensiveTaxInfo = comprehensiveTaxResults.find(
            (t) => t.owner === owner,
          );
          const propertyTaxInfo = propertyTaxResults.find(
            (t) => t.owner === owner,
          );
          const strategyInfo = strategies[owner];

          const totalTax =
            (propertyTaxInfo?.totalPayableAmount ?? 0) +
            (comprehensiveTaxInfo?.taxAmount ?? 0);

          return (
            <AccordionItem
              value={owner}
              key={owner}
              className="border bg-white dark:bg-slate-900 rounded-lg shadow-sm"
            >
                        ...
                        <CollapsibleTrigger className="w-full text-center">
                          ...
                          <p className="text-sm text-slate-500 dark:text-slate-400">
                            ...
                            <span className="font-semibold text-slate-900 dark:text-slate-100">
                              {propertyTaxInfo?.totalPayableAmount?.toLocaleString() ??
                                "0"}{" "}
                              원
                            </span>
                          </p>
                          {propertyTaxInfo?.referenceAmount && (
                            <p className="text-xs text-red-500 dark:text-red-400">
                              (임대사업 아닐 시:{" "}
                              {propertyTaxInfo.referenceAmount.toLocaleString()}{" "}
                              원)
                            </p>
                          )}
                        </CollapsibleTrigger>
                      ...
                      <Collapsible className="w-full p-4 rounded-lg bg-slate-50 dark:bg-slate-800 space-y-2">
                        <CollapsibleTrigger className="w-full text-center">
                          ...
                          <p className="text-sm text-slate-500 dark:text-slate-400">
                            총 납부액:{" "}
                            <span className="font-semibold text-slate-900 dark:text-slate-100">
                              {comprehensiveTaxInfo?.taxAmount?.toLocaleString() ??
                                "0"}{" "}
                              원
                            </span>
                          </p>
                          {comprehensiveTaxInfo?.referenceAmount && (
                            <p className="text-xs text-red-500 dark:text-red-400">
                              (임대사업 아닐 시:{" "}
                              {comprehensiveTaxInfo.referenceAmount.toLocaleString()}{" "}
                              원)
                            </p>
                          )}
                        </CollapsibleTrigger>
