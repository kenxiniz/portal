...
export async function GET() {
  ...
  if (!KAKAO_ACCESS_TOKEN) {
    return NextResponse.json(
      {
        error:
          "카카오 Access Token이 설정되지 않았습니다. .env 파일을 확인하세요.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const me: UserProfile = {
      ...
      profile_nickname:
        meResponse.data.kakao_account?.profile?.nickname || "이름 없음",
      profile_thumbnail_image:
        meResponse.data.kakao_account?.profile?.thumbnail_image_url || "",
