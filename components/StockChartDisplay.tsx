...
>(({ data, signals, gridStrokeColor, loading, error }, ref) => {
  ...
  useEffect(() => {
    ...
    const mainChart = createChart(chartContainerRef.current, {
      ...
      localization: {
        ...
        timeFormatter: (time: Time) =>
          new Date(time as string).toLocaleDateString("ko-KR", {
            year: "numeric",
            month: "long",
            day: "numeric",
          }),
    ...
    upperBandSeries.setData(
      data
        .filter((d) => d.bollingerBands)
        .map((d) => ({ time: d.date as Time, value: d.bollingerBands!.upper })),
    );
    lowerBandSeries.setData(
      data
        .filter((d) => d.bollingerBands)
        .map((d) => ({ time: d.date as Time, value: d.bollingerBands!.lower })),
    );
