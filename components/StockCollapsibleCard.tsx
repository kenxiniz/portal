...
export const StockCollapsibleCard: React.FC<StockCollapsibleCardProps> = ({
  ...
  const latestSignal =
    tickerState.signals.length > 0 ? tickerState.signals.at(-1) : null;
  ...
  const historicalSignals = tickerState.signals.filter(
    (s) => s.type !== "hold",
  );

  const handleSignalClick = (signal: TradingSignal) => {
    const targetDate =
      signal.date; /* 기간이 있더라도 항상 마지막 날짜를 기준으로 이동 */
    ...
    <Collapsible open={isOpen} onOpenChange={onOpenChange} className="w-full">
      <Card className="w-full bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 shadow-lg rounded-lg py-0">
        <CollapsibleTrigger asChild>
          <CardHeader
            className={cn(
              "flex flex-row justify-between items-center cursor-pointer rounded-t-lg transition-colors py-4 px-4",
            )}
          >
            <CardTitle
              className={cn("text-xl font-semibold", getCardTitleClassName())}
            >
              {ticker}
            </CardTitle>
                    ...
                    {historicalSignals.map((signal, index) => (
                      ...
                      >
                        <TableCell className="font-mono text-xs whitespace-nowrap py-1">
                          {signal.startDate
                            ? `${formatDate(signal.startDate)} ~ ${formatDate(signal.date)}`
