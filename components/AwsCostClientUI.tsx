    ...
    <div className="flex flex-col items-center p-4 md:p-8 bg-slate-100 dark:bg-slate-950 min-h-screen">
      ...
      <Card className="w-full max-w-4xl">
        <CardHeader>
          ...
          <CardDescription>
            연초부터 현재까지의 월별, 서비스별 상세 사용 내역입니다.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {initialData.map((monthData) =>
                Object.entries(monthData.services).map(
                  ([serviceName, serviceData], serviceIndex) =>
                    serviceData.details.map((detail, detailIndex) => (
                      <TableRow
                        key={`${monthData.month}-${serviceName}-${detailIndex}`}
                      >
                        {serviceIndex === 0 && detailIndex === 0 && (
                          <TableCell
                            rowSpan={Object.values(monthData.services).reduce(
                              (acc, s) => acc + s.details.length,
                              0,
                            )}
                        ...
                        {detailIndex === 0 && (
                          <TableCell
                            rowSpan={serviceData.details.length}
            ...
            <TableFooter>
              <TableRow className="bg-slate-100 dark:bg-slate-800 text-lg">
                <TableHead colSpan={3} className="font-bold">
                  연간 총계
                </TableHead>
