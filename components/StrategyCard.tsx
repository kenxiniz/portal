...
const getIconForStep = (step: string) => {
  if (step.includes("유지") || step.includes("보유"))
    return <Home className="h-5 w-5 text-blue-500" />;
  if (step.includes("매도") || step.includes("수익 실현"))
    return <Target className="h-5 w-5 text-green-500" />;
  if (step.includes("이주") || step.includes("입주") || step.includes("정착"))
    return <Key className="h-5 w-5 text-purple-500" />;
  if (step.includes("비과세") || step.includes("절감") || step.includes("혜택"))
